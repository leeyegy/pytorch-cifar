...
parser.add_argument('--apply_FN', default=False, action='store_true',
                    help='')
parser.add_argument('--gamma', default=1.0,type=float,
                    help='weight before feature loss')
...
save_path = os.path.join("checkpoint","feature_trades_"+args.net,str(args.apply_FN)+"_beta_"+str(args.beta)+"_gamma_"+str(args.gamma))
exp_name = os.path.join("runs", "feature_trades_"+args.net,str(args.apply_FN)+"_beta_"+str(args.beta)+"_gamma_"+str(args.gamma))
...
net_dict = {"VGG19":VGG('VGG19'),
            ...
            "Decouple18":Decouple18(apply_FN=args.apply_FN)
}
...
def train(args, model, device, train_loader, optimizer, epoch):
    model.train()
    mean_loss,mean_loss_adv,mean_loss_cln,mean_loss_feature=0,0,0,0
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        #                                    beta=args.beta)
        loss, loss_natural, loss_robust, loss_feature = trades_feature_loss(model=model,
                                           x_natural=data,
                                           y=target,
                                           optimizer=optimizer,
                                           step_size=args.step_size,
        ...
        # loss related
        mean_loss += loss.item()
        mean_loss_cln += loss_natural.item()
        mean_loss_adv += loss_robust.item()
    ...
    writer.add_scalars("train_loss_sub_items",{"adv_loss":mean_loss_adv/len(train_loader),"cln_loss":mean_loss_cln/len(train_loader),"feature_loss":mean_loss_feature/len(train_loader)},epoch)
