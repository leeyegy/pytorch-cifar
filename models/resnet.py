...
class ResNet_cosine(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        super(ResNet_cosine, self).__init__()
        self.in_planes = 64
...
class ResNet_Representation(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        super(ResNet_Representation, self).__init__()
        self.in_planes = 64
...
class ResNet_auxiliary(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10,auxiliary_num_classes=4):
...
class ResNet(nn.Module):
    ...
    def layer4_feature(self,x):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out_4_0 = self.layer4[0](out)
        out_4_1 = self.layer4[1](out_4_0)
        return out_4_0, out_4_1
...
class DecoupleNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10,apply_FN=False):
        ...
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.linear = nn.Linear(512*block.expansion, num_classes,bias=False) # notice here; differ from resnet
...
def ResNet18_aux():
    return ResNet_auxiliary(BasicBlock, [2, 2, 2, 2])

def ResNet18(num_classes=10,pretrained=False, model_root=None):
