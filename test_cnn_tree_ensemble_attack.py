...
from attack.PGDAttack import TreeAttack,TreeEnsembleAttack
...
net_4 = ResNet18(num_classes=4)
net_4 = net_4.to(device)

if device == 'cuda':
    net = torch.nn.DataParallel(net)
    net_6 = torch.nn.DataParallel(net_6)
    net_4 = torch.nn.DataParallel(net_4)
...
checkpoint = torch.load(os.path.join("checkpoint","mart_6_ResNet18","234567_beta_6.0","ckpt.pth"))
net_6.load_state_dict(checkpoint['net'])

checkpoint = torch.load(os.path.join("checkpoint","mart_4_ResNet18","0189_beta_6.0","ckpt.pth"))
net_4.load_state_dict(checkpoint['net'])

criterion = nn.CrossEntropyLoss()

adversary = TreeEnsembleAttack(main_model=net,attached_model=net_6,attached_model_left=net_4,eps=args.epsilon,perturb_steps=20,step_size=args.epsilon/10,beta=args.beta)

def gen_ori_tree_prediction(data,targets):
    # set state
    net.eval()
    net_6.eval()
    ...
    mask = (ori_prediction<2) | (ori_prediction>7) # 仅仅应用于0 1 8 9

    retest_data = data[mask].clone().detach()

    with torch.no_grad():
        output = net_4(retest_data)
        refined_prediction = torch.max(output,1)[1]
        map_mask =  refined_prediction>1
        refined_prediction[map_mask] = refined_prediction[map_mask] + 6
        tree_prediction[mask] = refined_prediction
...
def test():
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        ori_cln_prediction,tree_cln_prediction = gen_ori_tree_prediction(inputs,targets)
        ...
        ori_adv_prediction,tree_adv_prediction = gen_ori_tree_prediction(pgd_data,targets)
